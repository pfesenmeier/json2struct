...
impl Parser {
    ...
    pub fn set_camel(&mut self, camel: String) {
        self.camel = camel
    }
    ...
    pub fn parse(&mut self, params: &Value, struct_name: &str) -> String {
            let struct_header = format!(
                "{}\n{}\n{} struct {} {}",
                self.camel, self.derive, self.public, struct_name, "{"
            );
    ...
    fn is_object(&mut self, params: &Value) -> (Vec<String>, String) {
        ...
        for key_val in cur_map.iter() {
            ...
            if val.is_object() {
                if data.2 {
                    cur_struct = self.is_ok(&cur_type, &came_key, val, ok)
                }
                ...
                if !cur.is_empty() {
                    ...
                    println!("{:?}", &came_key);
                    cur_struct = self.is_ok(&cur_type, &came_key, cur_val, ok)
                }
                println!("{:?}", &cur_struct);
            }
